...
public class Hotel {
	...
	public void addServiceCharge(int roomId, ServiceType serviceType, double cosst) {
		Booking booking = activeBookingsByRoomId.get(roomId);
		if (booking == null) {
			String mesg = String.format("Hotel: addServiceCharge: no booking present for room id : %d", roomId);
			throw new RuntimeException(mesg);
		}
		booking.addServiceCharge(serviceType, cosst);
	}
	...
	public void checkout(int roomId) {
		...
		if (booking == null) {
			...
			throw new RuntimeException(mesg);
                ...
                activeBookingsByRoomId.remove(booking.getRoomId());
		booking.checkOut();
	}
